//Tarea 1. cambios de color del titulo
function colorTitle(selector) {
	$(selector).animate({
		color:'White'})
		.animate({
      color:'yellow'
		...
		.animate({
			...
			step: function () {
				...
				colorTitle('h1.main-titulo');
			},
			queue: true
		});
}

//Tarea 2. numeros aleatorios
...
// columnas y filas de dulces
function candyArrays(arrayType, index) {
...
function candyRows(index) {
	var candyRow = candyArrays('rows', index);
	return candyRow;
}

// Colunmnas
function candyColumns(index) {
	var candyColumn = candyArrays('columns');
	return candyColumn[index];
}

//Tarea 3. Eliminación de dulces en columnas
...
// Eliminación de dulces en filas
...
//Inclusión de caramelos al tablero
function checkBoard() {
	fillBoard();
}
...
//Tareas 7. drag and drop
//efecto de movimiento entre los caramelos
function candyEvents() {
	$('img').draggable({
...
//Tareas 4 y 6. temporizador y boton reiniciar
